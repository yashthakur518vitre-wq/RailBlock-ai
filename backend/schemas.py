...
def _validate_hhmm(value: str, field: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"{field} must be in HH:MM format (00:00–23:59), got '{value}'")
    return value
...
class DepartmentCreate(BaseModel):
    ...
    name: str = Field(..., min_length=1, description="Full department name")
...
class CorridorCreate(BaseModel):
    ...
    block_id: str = Field(..., min_length=1, description="Unique block ID, e.g. B001")
    block_name: str = Field(..., min_length=1)
    start_station: str = Field(..., min_length=1)
    end_station: str = Field(..., min_length=1)
    line_capacity_per_day: int = Field(default=1, ge=1,
                                       description="Number of train paths per day")
    annual_gmt: Optional[float] = Field(default=None, ge=0.0,
...
class ResourceCreate(BaseModel):
    ...
    department_code: str = Field(..., description="Department this resource belongs to")
    home_depot: Optional[str] = Field(default=None, description="Home base / depot name")
    resource_type: str = Field(..., description="machine | crew | vehicle | equipment")
    ...
    def validate_availability(cls, v: str) -> str:
        ...
        if v not in VALID_AVAILABILITY:
            raise ValueError(f"availability_status must be one of {VALID_AVAILABILITY}")
        return v
...
class AvailabilityWindowCreate(BaseModel):
    ...
    corridor_block_id: str = Field(..., description="Block ID of the corridor")
    date: date
    start_time: str = Field(..., description="Window start in HH:MM format")
    end_time: str = Field(..., description="Window end in HH:MM format")
    ...
    def end_after_start(self) -> AvailabilityWindowCreate:
        def to_min(t: str) -> int:
            h, m = map(int, t.split(":"))
            return h * 60 + m
        if to_min(self.end_time) <= to_min(self.start_time):
...
class TrainCreate(BaseModel):
    ...
    train_id: str = Field(..., min_length=1, description="Train number, e.g. '12001'")
...
class TrainOccupancyCreate(BaseModel):
    ...
    train_id: str = Field(..., description="Train number matching an existing train")
    corridor_block_id: str = Field(..., description="Block ID of the corridor")
    date: date
    entry_time: str = Field(..., description="Train entry time HH:MM")
    exit_time: str = Field(..., description="Train exit time HH:MM")
    source: str = Field(default="timetable",
    ...
    @model_validator(mode="after")
    def exit_after_entry(self) -> TrainOccupancyCreate:
        def to_min(t: str) -> int:
            h, m = map(int, t.split(":"))
            return h * 60 + m
        if to_min(self.exit_time) <= to_min(self.entry_time):
            raise ValueError("exit_time must be strictly later than entry_time")
        return self
...
class MaintenanceTaskCreate(BaseModel):
    ...
    department_code: str = Field(..., description="Department code (ENG/SNT/TD)")
    corridor_block_id: str = Field(..., description="Block ID of the corridor")
...
class OptimizationSummary(BaseModel):
    ...
    coordinated_tasks: int      # tasks that share a block with another dept
    solve_time_ms: float
...
class GenerateBlockPlanRequest(BaseModel):
    ...
    def validate_pairs(cls, v: list[list[str]]) -> list[list[str]]:
        ...
        return [[a.strip().upper(), b.strip().upper()] for a, b in v]
...
class TmsTaskPayload(BaseModel):
    ...
    tms_ref: str
    section_id: str          # maps to corridor block_id
    defect_description: str
    defect_code: str         # TMS internal code → maps to defect_type
    severity: int = Field(..., ge=1, le=5)  # maps to criticality
    logged_date: date
    target_date: date
    work_hours: float = Field(..., gt=0)    # maps to estimated_duration_minutes
    track_impact_index: float = Field(default=0.0, ge=0.0)  # maps to asset_impact_score
...
class SmmsTaskPayload(BaseModel):
    ...
    reported_on: date
    due_on: date
    est_duration_hrs: float = Field(..., gt=0)
    signal_impact_score: float = Field(default=0.0, ge=0.0)
...
class TdmsTaskPayload(BaseModel):
    ...
    traction_impact: float = Field(default=0.0, ge=0.0)
...
class CoaWindowPayload(BaseModel):
    ...
    @model_validator(mode="after")
    def to_after_from(self) -> CoaWindowPayload:
        def to_min(t: str) -> int:
            h, m = map(int, t.split(":"))
            return h * 60 + m
        if to_min(self.to_time) <= to_min(self.from_time):
            raise ValueError("to_time must be strictly later than from_time")
        return self
...
class CoaOccupancyPayload(BaseModel):
    ...
    arrival_time: str
    departure_time: str
    train_type: str = Field(default="passenger",
    ...
    @model_validator(mode="after")
    def depart_after_arrive(self) -> CoaOccupancyPayload:
        def to_min(t: str) -> int:
            h, m = map(int, t.split(":"))
            return h * 60 + m
        if to_min(self.departure_time) <= to_min(self.arrival_time):
            raise ValueError("departure_time must be strictly later than arrival_time")
        return self
